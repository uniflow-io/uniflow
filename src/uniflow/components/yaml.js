...
let length = string.length
    ...
    scope.yamlWrite = function (yaml, key, value) {
        let nodeStack = []; let storeStack = {}; let indentationSpaceAdverage = [2]
...
let node = {
        ...
        scope.jsyaml.load(yaml, {
            'listener': listenerFunction
        })

        let root = storeStack[0][0]

let indentationSpace = Math.round(indentationSpaceAdverage.reduce(function (count, spaces) {
                return count + spaces
            }, 0) / indentationSpaceAdverage.length)

        let keys = key.split('.'); let keyIndex
        ...
        if (keyIndex === keys.length) {
            ...
            let indentation; let childsKeys = Object.keys(root.children)
...
let keyvaluelist = scope.jsyaml.load(yaml)
...
let keyvaluelist = runner.getValue(this.state.keyvaluevariable)
