...
class HistoryService
{
    ...
    public function resetDemoAccount(User $user)
    {
        ...
        if (true || $data === false || $now->format('Y-m-d') !== $data->format('Y-m-d')) {
            $data = $now;
