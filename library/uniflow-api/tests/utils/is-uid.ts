...
const unparse = (buf: Buffer, offset: number = 0): String => {
  let i = offset || 0, bth: Array<string> = _byteToHex;
...
const extractVersion = (uuid: string): Number => {
    return parseInt(uuid.charAt(14))|0;
};
 ...
 */
const checkUUID = (uuid: String|Buffer, version?: Number): Boolean => {
    let parsedUuid;
