  ...
  PropertyPath.prototype.getElement = function getElement(index) {
    if (typeof this._elements[index] === 'undefined') {
      throw new OutOfBoundsException(
        scope.format('The index %s is not within the property path', index)
      )
    }
...
;(function(scope) {
   ...
   * @extends Error
   */
  UnexpectedTypeException = function UnexpectedTypeException(
    value,
    expectedType
  ) {
    Error.call(
   ...
   *                                            nor array
   */
  PropertyAccessor.prototype.setValue = function setValue(
    objectArray,
    propertyPath,
    value
  ) {
   ...
   */
  PropertyAccessor.prototype.readPropertiesUntil = function readPropertiesUntil(
    objectArray,
    propertyPath,
    lastIndex,
    ...
    for (i = 0; i < lastIndex; i++) {
      ...
      if (
        isIndex &&
        isObject(objectArray) &&
        !objectArray.hasOwnProperty(property)
      ) {
   ...
   */
  PropertyAccessor.prototype.write = function write(
    objectArray,
    property,
    value
  ) {
