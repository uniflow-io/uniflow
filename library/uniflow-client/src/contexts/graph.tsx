...
export const GraphContext = React.createContext<{graph: GraphProviderState, graphDispatch: GraphDispath, graphRef: MutableRefObject<GraphProviderState>}>({
  graph: defaultState,
  graphDispatch: () => {
...
export function GraphProvider(props: GraphProviderProps) {
  const [graph, graphDispatch, graphRef] = useReducerRef<GraphProviderState, GraphAction>((
    state: GraphProviderState = defaultState,
    action: GraphAction
  ) => {
    switch (action.type) {
