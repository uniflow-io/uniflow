...
class Api {
  ...
  async updateFolder(path: {uid: UuidType}, body: Partial<Pick<FolderApiType, 'name'|'path'>&{slug: string}>, options: ApiOptions & Required<Pick<ApiOptions,'token'>>): Promise<FolderApiType> {
    try {
  ...
  async updateProgram(path: {uid: UuidType}, body: Partial<ProgramApiType&{slug: string}>, options: ApiOptions & Required<Pick<ApiOptions,'token'>>): Promise<ProgramApiType> {
    try {
