      ...
      <div>
          ...
          <form className='form-horizontal'>
            <div className='box-header with-border'>
              <h3 className='box-title'>
                <button type='submit' className='btn btn-default'>{this.state.running ?
                  <i className='fa fa-refresh fa-spin'/> : <i className='fa fa-refresh fa-cog'/>}</button>
                If
              </h3>
        ...
        <ListComponent stack={this.state.if.conditionStack} runIndex={this.state.if.conditionRunIndex}
                       ...
                       onPush={(index, component) => {
                         this.onPushFlow(['if', 'conditionStack'], index, component)
                       }}
                       onPop={index => {
                         this.onPopFlow(['if', 'conditionStack'], index)
                       }}
                       onUpdate={(index, data) => {
                         this.onUpdateFlow(['if', 'conditionStack'], index, data)
                       }}
                       onRun={null}/>
        <div className='box box-info'>
          <form className='form-horizontal'>
        ...
        <ListComponent stack={this.state.if.executeStack} runIndex={this.state.if.executeRunIndex}
                       ...
                       userComponents={this.props.userComponents}
                       onPush={(index, component) => {
                         this.onPushFlow(['if', 'executeStack'], index, component)
                       }}
                       onPop={index => {
                         this.onPopFlow(['if', 'executeStack'], index)
                       }}
                       onUpdate={(index, data) => {
                         this.onUpdateFlow(['if', 'executeStack'], index, data)
                       }}
                       onRun={null}/>
        {this.state.elseIfs.map((item, elseIfIndex) => (
          <div key={('elseIf' + elseIfIndex)}>
            ...
            <ListComponent stack={item.conditionStack} runIndex={item.conditionRunIndex}
                           ...
                           userComponents={this.props.userComponents}
                           onPush={(index, component) => {
                             this.onPushFlow(['elseIfs', elseIfIndex, 'conditionStack'], index, component)
                           }}
                           onPop={index => {
                             this.onPopFlow(['elseIfs', elseIfIndex, 'conditionStack'], index)
                           }}
                           onUpdate={(index, data) => {
                             this.onUpdateFlow(['elseIfs', elseIfIndex, 'conditionStack'], index, data)
                           }}
                           onRun={null}/>
            <div className='box box-info'>
              <form className='form-horizontal'>
            ...
            <ListComponent stack={item.executeStack} runIndex={item.executeRunIndex}
                           ...
                           userComponents={this.props.userComponents}
                           onPush={(index, component) => {
                             this.onPushFlow(['elseIfs', elseIfIndex, 'executeStack'], index, component)
                           }}
                           onPop={index => {
                             this.onPopFlow(['elseIfs', elseIfIndex, 'executeStack'], index)
                           }}
                           onUpdate={(index, data) => {
                             this.onUpdateFlow(['elseIfs', elseIfIndex, 'executeStack'], index, data)
                           }}
                           onRun={null}/>
        ...
        {this.state.else && (
          <div key={'else'}>
            <div className='box box-info'>
              <form className='form-horizontal'>
                <div className='box-header with-border'>
                  ...
                  <div className='box-tools pull-right'>
                    <button className='btn btn-box-tool' onClick={this.onRemoveElse}><i className='fa fa-times'/>
                    </button>
                  </div>
                </div>
              </form>
            ...
            <ListComponent stack={this.state.else.executeStack} runIndex={this.state.else.executeRunIndex}
                           ...
                           userComponents={this.props.userComponents}
                           onPush={(index, component) => {
                             this.onPushFlow(['else', 'executeStack'], index, component)
                           }}
                           onPop={index => {
                             this.onPopFlow(['else', 'executeStack'], index)
                           }}
                           onUpdate={(index, data) => {
                             this.onUpdateFlow(['else', 'executeStack'], index, data)
                           }}
                           onRun={null}/>
          </div>
        )}
